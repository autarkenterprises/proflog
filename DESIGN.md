...
- *Parameter structural isolation*: `(par p)` cannot participate in `free-closureo` or `decompose-eq-argso` (it's `['par nom]`, not `(lcons 'app ...)`). This is sound: parameters represent arbitrary domain elements and should not be assumed distinct from constructors. Tests ADV12/12b verify this.
