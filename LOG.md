...
- Fast-forward merged `adr-0060-willard-sjas-mvp` into
  `review/sjas-lang-profile-design`, completing [ADR-0060](docs/adr/ADR-0060-willard-sjas-mvp.md)
  with `proflog.willard-sjas`, `proflog.kernel.willard-sjas-profile`,
  `lein test-proflog-sjas`, normalized bounded-quantifier lowering, worked
  examples, and guide/README updates. Added frontend `(forall<= [x] bound φ)` /
  `(exists<= [x] bound φ)` plus `validate-formula` support so bounded clauses
  compile like SJAS AST. See [AAR-0060](docs/aar/AAR-0060-willard-sjas-mvp.md).
  Verification figures recorded there (`lein test-proflog-fast`,
  `lein test-proflog-extended`, focused normalize selector).
