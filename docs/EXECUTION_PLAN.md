...
| [ADR-0018](adr/ADR-0018-existential-disequality-witnesses.md) | accepted | `adr-0018-existential-disequality-witnesses` | accurate object-language witnesses for existential disequality programs | ADR-0017 | gatekeeping regressions for `p(x) :- exists y. x != y` over `{a,b}`: `p(a)` and `p(b)` succeed, open answers are exactly `a` and `b`, and no `(par ...)` escapes | greenfield evaluates the existential disequality witness program accurately without imitating legacy's impure `project`-based answer leak; fast and legacy-impurity suites pass |
