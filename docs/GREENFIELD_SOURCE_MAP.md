...
| `proflog.kernel.robinson-q-profile` | Deduction-modulo and case-split profile for Robinson Q. | Binds a miniKanren theory rule into `kernel/close-agendao`; it normalizes visible `add` and `mul` terms by Q conversion rules, records Q3's predecessor-or-zero branch closure with `q3-case-split`, and wraps theory closures with `profiled robinson-q`. Q3 is intentionally not a rewrite. |
