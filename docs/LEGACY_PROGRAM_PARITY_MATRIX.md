...
| `append` / `reverse` list relations | [Y01-Y12](../test/cljtap/alphaleantap_ep_test.clj) | [list_programs_test.clj](../test/proflog/list_programs_test.clj), [synthesis_modes_test.clj](../test/proflog/synthesis_modes_test.clj), [answers_test.clj](../test/proflog/answers_test.clj), [parity_test.clj](../test/proflog/parity_test.clj) | Partial | Legacy still completes `Y10` and `Y12` in about `27-28 s`. Greenfield now has two distinct answer tracks: the default generic path remains symbolic and still tops out at `r = []` then `r = [a]` for `reverse([a,b], r)`, while ADR-0012 adds `query-parity-answers` for dedicated closed-answer materialization. That parity mode now exports `reverse([a,b], r) = [b,a]` and all four inverse `append` splits without changing the generic symbolic contract. The generic path itself is still below legacy parity, so the overall family remains partial until ADR-0013 improves the relational engine rather than the specialty mode. |
| Nested and deep `append` list families | [Y13-Y15](../test/cljtap/alphaleantap_ep_test.clj), [Z01-Z04](../test/cljtap/alphaleantap_ep_test.clj) | [list_programs_test.clj](../test/proflog/list_programs_test.clj), [synthesis_modes_test.clj](../test/proflog/synthesis_modes_test.clj), [parity_test.clj](../test/proflog/parity_test.clj) | Partial | Legacy still completes `Y15` (`17.76 s`) and `Z04` (`17.06 s`) on the current machine. Greenfield now preserves the generic symbolic suffix frontier honestly at the default raw cap, while the dedicated parity mode materializes the closed nested inverse and nested suffix answers separately. Depth-3 forward answer synthesis remains beyond parity, and the generic path still does not subsume the specialty mode. |
