...
| `append` / `reverse` list relations | [Y01-Y12](../test/cljtap/alphaleantap_ep_test.clj) | [list_programs_test.clj](../test/proflog/list_programs_test.clj), [synthesis_modes_test.clj](../test/proflog/synthesis_modes_test.clj), [answers_test.clj](../test/proflog/answers_test.clj) | Partial | Legacy still completes `Y10` and `Y12` in about `27-28 s`. Greenfield ground checks are fine, and the default open-answer path now enters top-level literal queries directly in the kernel, prefers descent before deferral while `call-depth` remains, and ranks later closed answers ahead of earlier symbolic frontiers. Operationally `reverse([a,b], r)` now refines from `r = []` at `call-depth 0` to `r = [a]` at `call-depth 1`, and inverse `append` again prefers the base split plus the first concrete recursive split. Full legacy parity is still missing because `reverse` still did not export `[b,a]` in a `>120 s`, `fuel 64`, `call-depth 3`, raw-`64` probe, and inverse `append` still does not reach the deeper `([a,b],[c])` and `([a,b,c],[])` splits in the default slice. |
| Nested and deep `append` list families | [Y13-Y15](../test/cljtap/alphaleantap_ep_test.clj), [Z01-Z04](../test/cljtap/alphaleantap_ep_test.clj) | [list_programs_test.clj](../test/proflog/list_programs_test.clj), [synthesis_modes_test.clj](../test/proflog/synthesis_modes_test.clj) | Partial | Legacy still completes `Y15` (`17.76 s`) and `Z04` (`17.06 s`) on the current machine. Greenfield now preserves the generic symbolic suffix frontier honestly at the default raw cap, and a longer raw-proof probe (`max-raw-proof-limit 64`) can recover the concrete nested suffix answer `[[c]]`. Inverse nested splits beyond that and the depth-3 forward answer family remain beyond practical parity. |
