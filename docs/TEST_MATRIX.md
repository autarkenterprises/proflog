...
| Proof profiles and deduction modulo | language-level profile metadata, profile dispatch, kernel-interleaved theory conversion, Q3 predecessor introduction, and case-split proof markers | Robinson Q as ordinary assumptions and as `:robinson-q` conversion/full-Q3 profile | common theorem timing comparison, source audit against host preprocessing, and proof evidence distinguishing assumptions from interleaved Q conversion, Q3 case split, and full-Q3 predecessor use | yes when production query dispatch changes |
