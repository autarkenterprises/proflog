...
| Proof profiles and deduction modulo | language-level profile metadata, profile dispatch, kernel-interleaved theory conversion and case-split proof markers | Robinson Q as ordinary assumptions and as `:robinson-q` conversion/case-split profile | common theorem timing comparison, source audit against host preprocessing, and proof evidence distinguishing assumptions from interleaved Q conversion and Q3 case split | yes when production query dispatch changes |
