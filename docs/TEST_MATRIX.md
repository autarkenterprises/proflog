...
| Proof profiles and deduction modulo | language-level profile metadata, profile dispatch, theory conversion and case-split proof markers | Robinson Q as ordinary assumptions and as `:robinson-q` conversion/case-split profile | common theorem timing comparison and proof evidence distinguishing assumptions from conversion and Q3 case split | yes when production query dispatch changes |
