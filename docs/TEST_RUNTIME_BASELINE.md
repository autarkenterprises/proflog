...
| `query-answers-collect-unique-answers-beyond-duplicate-proof-paths` | `proflog.answers-test` | duplicate `dup(x)` proofs for `0` before distinct answer `1` | `36.77 s` | Answer export now keeps searching for unique records and rejects impossible residuals such as `neq(0, 0)`. |
