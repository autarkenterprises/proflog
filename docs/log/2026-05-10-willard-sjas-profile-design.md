...
- Limited theorem reuse. A faithful ISD(A) or IS#_D(beta) implementation needs
  the Tab-1 proof-list apparatus, not just ordinary tableaux.
...
For the Proflog profile, this means `:proof-profile :willard-sjas-level1`
cannot be a cosmetic language tag. It must select a concrete proof apparatus
and make proof evidence record that apparatus.
...
- a first SJAS profile may support ordinary semantic-tableaux reflection;
- a faithful ISD(A) or IS#_D(beta) profile must also implement a limited
  `Tab-1` proof-list relation, or explicitly state that it demonstrates the
  semantic-tableaux substrate but not the full Tab-1 apparatus.
...
- relation/formula builders for `Mult`, bounded quantifiers, Delta-star-0,
  Pi-star-1, proof predicates, and `SelfCons1`;
...
- the generated `SelfCons1` formula mentions the selected system id and proof
  apparatus.
...
### Slice 4: Limited Tab-1 Apparatus
...
### Slice 5: Finite SJAS Demonstrator
...
- A profile without Tab-1 theorem reuse should not be called a faithful ISD(A)
  implementation. It can be called a semantic-tableaux SJAS substrate.
...
Use `:willard-sjas-level1` for the first profile.

This name says three important things:
...
- it targets Level-1 semantic-tableaux consistency, not Hilbert consistency or
  the later theta-function profile.
...
- proof-certificate relations accept valid and reject invalid miniature tableau
  proofs;
- Tab-1 proof-list restrictions are enforced if the ADR claims ISD(A) or
  IS#_D(beta);
- the finite SJAS demonstrator proves its intended self-consistency statement;
