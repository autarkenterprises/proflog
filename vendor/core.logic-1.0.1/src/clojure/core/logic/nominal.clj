            ...
            ((composeg*
               ...
               (fn [a]
                 (cond
                   ...
                   :else ;; (= t1 t2)
                   ;; proflog ADR-0147: a failing freshness constraint returns
                   ;; nil (failure); without this guard the next iteration
                   ;; walks the nil state and throws instead of failing the
                   ;; suspension. nil short-circuits, exactly as composeg does.
                   (loop [a* swap a a]
                     (cond
                       (nil? a) nil
                       (empty? a*) a
                       :else (recur (rest a*) ((hash (first a*) t2) a))))))) a))
          IRunnable
          (-runnable? [_]
